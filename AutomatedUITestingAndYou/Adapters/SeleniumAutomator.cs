...
namespace AutomatedUITestingAndYou.Adapters
{
    public class SeleniumAutomator : IBrowserAutomator
    {
        ...
        public void SetResolution(int width, int height) => WebDriver.Manage().Window.Size = new Size(width, height);

        public void ScrollUp() => new Actions(WebDriver).SendKeys(Keys.PageUp).Perform();

        public void ScrollDown() => new Actions(WebDriver).SendKeys(Keys.PageDown).Perform();
        ...
        private SeleniumElement GetSeleniumElement(IPageElement element)
        {
            ...
            {
                throw new ArgumentException($"Element is not compatible with the Selenium Framework");
            }
